...
std::vector<DimArg> texprDims(const torch::jit::Value* v) {
...
Tensor* TensorExprKernel::ComputeValue(const torch::jit::Value* v) {
  switch (v->node()->kind()) {
    ...
    default: {
      ...
      auto qs = v->node()->kind().toQualString();
      if (nfr.count(qs)) {
        return nfr.at(qs).second(this, v);
      }
...
void TensorExprKernel::addNoInline(int64_t unique_id) {
  no_inline_.insert(unique_id);
}
...
void TensorExprKernel::LowerToBackend(BackendType backend_type) {
  ...
  for (auto& p : tensors_) {
    if (dynamic_cast<const CallExternal*>(p.second->body()) != nullptr) {
      addNoInline(p.first);
    }
  }
  ...
  for (auto& p : tensors_) {
    if (no_inline_.find(p.first) != no_inline_.end()) {
      continue;
    }
  ...
  if (backend_type == kCudaCodeGen) {
    for (int i = 0; i < tensor_outputs_.size(); i++) {
      // TODO: audit this logic in the presence of external calls
