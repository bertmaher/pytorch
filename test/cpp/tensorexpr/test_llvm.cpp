...
void testLLVMSimpleMath01() {
  ...
  Stmt* stmt = sch.Lower();
  Buffer f_buf(VarHandle(tensor->function()->func_var(tensor->output_index())), kFloat32, {N});
...
void testLLVMComputeMul() {
  ...
  Buffer c_buf(VarHandle(c->function()->func_var(c->output_index())), kFloat32, {N});
...
void testLLVMBroadcastAdd() {
  ...
  Buffer c_buf(VarHandle(c->function()->func_var(c->output_index())), kFloat32, {M, N});
  Schedule sch = Schedule::make({c});
  Stmt* s = sch.Lower();
