...
class TensorExprFusedKernel : public ::torch::jit::fuser::FusedKernel {
 public:
  ...
  void launch_raw(const uint32_t numel, std::vector<void*>& arguments) const override {
    ...
    std::vector<tensorexpr::CodeGen::CallArg> runArgs;
    for (size_t i = 0; i < inputDesc().size(); i++) {
      auto const nDim = inputDesc()[i].nDim();
      auto const ti = reinterpret_cast<TensorInfo*>(arguments[i + 1]);
      runArgs.push_back(ti->data);
      for (size_t j = 0; j < nDim; j++) {
        runArgs.push_back((int32_t)ti->sizes(nDim)[j]);
      }
    }
    for (size_t i = 0; i < outputDesc().size(); i++) {
      auto const nDim = outputDesc()[i].nDim();
      auto const ti = reinterpret_cast<TensorInfo*>(arguments[inputDesc().size() + 1 + i]);
      runArgs.push_back(ti->data);
    }
    kernel_->CodeGenRun(runArgs);
  }
...
static std::shared_ptr<FusedKernel> createTexprKernel(
  ...
  return std::make_shared<TensorExprFusedKernel>(
      kernel,
      device, name, "<generated by tensorexpr>",
